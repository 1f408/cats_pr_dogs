...
const AppConfigUnipath = "/conf/etc/tmplview.conf"
const AppName = "CatPrTmpl"
...
func main() {
	cfg, err := tmplview.NewTmplViewConfigFS(WebViewFS, AppConfigUnipath)
	if err != nil {
	...
	cfg.Tmpl.DocumentRoot = FsDocumentRootUpath
	cfg.Tmpl.DirectoryViewRoots = []upath.UPath{FsDocumentRootUpath}
	...
	cfg.Tmpl.CatUiConfigPath = upath.Zero
	...
	tmplv, verr := tmplview.NewTmplView(cfg)
	if verr != nil {
		fmt.Fprintln(os.Stderr, verr)
		os.Exit(1)
	}
	SkipWatchFileRe = tmplv.DirectoryViewHidden

	tmpl_handle := NewRewriteHeaderHandler(http.HandlerFunc(tmplv.Handler))
	...
	http.Handle(ViewTopUrl, http.StripPrefix(ViewTopUrl, tmpl_handle))
	...
	set_auth_account := func(name string) {
		authNameMtx.Lock()
		defer authNameMtx.Unlock()

		authName = name
		tmpl_handle.SetRewrite(tmplv.AuthnUserHeader, name)
	}
	ui.Bind("set_auth_account", set_auth_account)

	get_auth_account := func() string {
		authNameMtx.Lock()
		defer authNameMtx.Unlock()

		return authName
	}
	ui.Bind("get_auth_account", get_auth_account)
